...
class TextAnalyzer:
    """
    A class for analyzing text files using TextRank-style keyword extraction,
    keyword deduplication with Claude, and co-occurrence graph generation with Ray parallelization.
    """
    
    def __init__(self, file_path: str):
        """
        Initialize the TextAnalyzer with a file path.
    ...
    def _candidate_tokens(self, text, *, use_lemmatize=True, remove_stopwords=True, min_len=2):
        """
        Return:
    ...
    def deduplicate_keywords_with_claude(self, keywords):
        """
    ...
    def consolidate_graph(self, G, clusters):
        """
        ...
        if new_G.number_of_nodes() > 0:
            self.draw_graph(new_G, title="Consolidated Graph")
        else:
    ...
    def run_parallel_pipeline(
        ...
        *,
        ...
        partial_map = self._merge_keyword_maps([r["keyword_map"] for r in results])
        ...
        if final_global_dedup:
            ...
            keyword_map = self.json_to_dict(dedup_json)
        else:
    ...
    def analyze(
        ...
        **plot_kwargs
        ...
        Args:
            ...
            hard_max_chars: Maximum characters per chunk
            overlap_sentences: Number of overlapping sentences between chunks
            max_chunks: Maximum number of chunks to process
            num_cpus: Number of CPUs for Ray (None for auto-detect)
            window: Window size for co-occurrence
        ...
        if plot_graph:
            ...
            plot_params = {
                ...
                'min_weight': 1,
                'layout': "spring",
                'title': f"Co-occurrence Graph for {self.file_path}",
                'save_path': None,
                'show_weights': False,
            }
            plot_params.update(plot_kwargs)
            
            self.plot_graph(adj, **plot_params)
...
# Example usage
def main():
    # Create analyzer instance with file path
    analyzer = TextAnalyzer("articles/sorting_algorithms.txt")
    
    try:
        # Run analysis with Ray parallelization
